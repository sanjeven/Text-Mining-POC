...
_URL = 'https://www.theguardian.com/books/2003/oct/12/features.fiction'
...
class Lit(object):
    ...
    def wiki_scrapping(self):
        ...
        FIND_AUTHOR = soup.find_all('p')
        print FIND_AUTHOR
        
        with open (_FILE_NAME,"ab") as logFile:
            logFileWriter = csv.writer(logFile)
            logFileWriter.writerow([FIND_AUTHOR])
        
        '''
        for div in soup.find_all("div", { "class" : "content__article-body from-content-api js-article__body" }):
            #print div
            try:
                FIND_AUTHOR = div.find('p').text.encode('ascii', 'replace').decode('ascii')
                print FIND_AUTHOR
        ...
        '''
        try:
            FIND_AUTHOR = soup.find('li').text.encode('ascii', 'replace').decode('ascii')
            with open (_FILE_NAME,"ab") as logFile:
                logFileWriter = csv.writer(logFile)
                logFileWriter.writerow([FIND_AUTHOR])
            print FIND_AUTHOR
